...
def prob_cuts_3d(lbound, rbound, trials):
    m = MIE(identity_matrix(RR, 3), vector(RR, [0, 0, 0]))
    m.integrate_parallel_cuts_hint(vector(RR, [1, 0, 0]), lbound, rbound)
    mean = [0, 0, 0]
    ...
    for ind in range(trials):
        point = np.random.multivariate_normal(mean, cov, int(1))
        if (((point[0][0]-m.mu[0])/a)^2 + ((point[0][1]-m.mu[1])/b)^2 + ((point[0][2]-m.mu[2])/c)^2) <= 1:
            list_points.append([point,1])
            inMIE += 1
        elif ((point[0][0])^2 + (point[0][1])^2 + (point[0][2])^2) <= 1 and point[0][0] <= rbound and point[0][0] >= lbound:
            list_points.append([point,0])
            inEllipse += 1
    print(f'Vol Lost = {1-N(inMIE/(inMIE+ inEllipse))}')
    S = Sphere(1, opacity = 0.5, color = "yellow").scale(1,1,1) + Sphere(1, opacity = 0.5).scale(m.S[0][0],m.S[1][1],m.S[2][2])
    for samp_point in list_points:
        if samp_point[1] == 0:
            S += point3d(samp_point[0],size=10,color='black',opacity=.5)
        else:
            S += point3d(samp_point[0],size=10,color='white',opacity=.5)
    H.<x,y,z> = HyperplaneArrangements(RR)
    S += (x - 0.5).plot(opacity = 0.5,color = "gray")
    S += (x + 0.5).plot(opacity = 0.5, color = "grey")
    S.show()
prob_cuts_3d(-0.5,0.5,1000)
